...
import storageManager, { heighbrushColor, heighpencilColor, heighpencilSize } from "~utils/storage";
...
const PlasmoOverlay = () => {
  ...
  const toggleHighlighting = () => {
    hideDrawingCanvas(canvasObject.current);
  ...
  const activatePen = (param: { brush?: number, medium?: number }) => {
    ...
    drawingBrush.current.color = brushColors[brush || pencilBrush];
    drawingBrush.current.width = medium || pencilMedium;
  ...
  useEffect(() => {
    ...
    const initData = async () => {
      ...
      try {
        const pencilColor = await storageManager.get(heighpencilColor);
      ...
      try {
        const pencilSize = await storageManager.get(heighpencilSize);
      ...
      ))}
      {
        isVisible && (
          <div className="plasmo-overlay">
            ...
            <div className="plasmo-overlay-item" onClick={() => setIsSetting(prev => !prev)}>
                  ...
                  <div className="top-section">
                    {
                      brushSizes.map((size, index) => (<div
                        key={`${size}-${index}`}
                        onClick={() => { storageManager.set({[heighpencilSize]: JSON.stringify(size)}); setPencilMedium(size); activatePen({ medium: size })}}
                  ...
                  <div className="bottom-section">
                    {
                      brushColors.map((color, index) => (<div
                        key={color}
                        onClick={() => { storageManager.set({[heighpencilColor]: JSON.stringify(index)}); setPencilBrush(index); activatePen({ brush: index })}}
